...
type Jwt interface {
	...
	AuthAccessToken(c *fiber.Ctx, redis redis.Redis, jwt Jwt, secret string) error
	AuthRefreshToken(c *fiber.Ctx, redis redis.Redis, jwt Jwt, secret string) error
...
func (j *jwt) Generate(claims Claims, secret string) (string, error) {
...
func (j *jwt) ExtractTokenFromHeader(c *fiber.Ctx) (string, error) {
	...
	if !strings.HasPrefix(authHeader, "Bearer ") {
		return "", errors.New("[jwt] : invalid token type")
	}
...
func (j *jwt) AuthAccessToken(c *fiber.Ctx, redis redis.Redis, jwt Jwt, secret string) error {
	authHeader := c.Get("Authorization")
	if !strings.HasPrefix(authHeader, "Bearer ") {
	...
	var tokenContext contextPkg.TokenContext
	...
	var userContext contextPkg.UserContext
...
func (j *jwt) AuthRefreshToken(c *fiber.Ctx, redis redis.Redis, jwt Jwt, secret string) error {
	authHeader := c.Get("Authorization")
	if !strings.HasPrefix(authHeader, "Bearer ") {
		return fmt.Errorf("[jwt] invalid token type")
	}
